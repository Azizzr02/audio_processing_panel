...
root = tk.Tk()
root.title("Audio Processing Control Panel")
root.geometry("800x600")
root.configure(bg="#EAB8E4")  # Background color

# Create main frame
main_frame = tk.Frame(root, padx=10, pady=10, bg="#EAB8E4")
main_frame.pack(padx=10, pady=10)

# Create a label
label = tk.Label(main_frame, text="Audio Processing Control Panel", font=("Helvetica", 20, "bold"), bg="#EAB8E4")
label.pack(pady=10)

# Text input for text-to-speech
input_text = tk.Text(main_frame, height=5, width=50, wrap=tk.WORD)
input_text.pack(pady=10)

# Create buttons
classify_button = tk.Button(main_frame, text="Audio Classification", command=classify_audio, width=20, bg="#FFB3C1", font=("Arial", 14))
classify_button.pack(pady=5)

record_button = tk.Button(main_frame, text="Record Audio", command=record_audio, width=20, bg="#FFB3C1", font=("Arial", 14))
record_button.pack(pady=5)

speak_button = tk.Button(main_frame, text="Convert to Speech", command=text_to_speech, width=20, bg="#FFB3C1", font=("Arial", 14))
